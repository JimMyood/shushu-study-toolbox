...
- 工具会在提交前复核公开 stale。隔离过程中若路径变化，命令停止且不更新清单；
  竞争文件与已建立的私有载体保持原样。失败归档中可能是旧译文，也可能是
  并发变化后的内容；必须同时核对正式译文路径、公开 stale 和提示的安全归档。
  安全归档不是路径锁，不承诺外部进程在事务一致点之后不会继续修改公开路径。
  如需修改归档或 stale，先复制到新文件，不要原地编辑。
- 上一版 manifest 没有 hash，无法证明 `.zh.txt` 与哪版源文绑定；重跑时会把所有
  旧译文条目以整批事务隔离为 `chunk_NNN.stale-legacy[-N].txt`，全部标记待重翻。
  任一条目失败会尽力逆序恢复已经移动的条目，并把已建立的私有载体
  原样保留供人工核对；即使源文看似未变也不得自动复用。
...
- `无法安全隔离第 N 块旧译文`：新版 `.translated.txt` 隔离失败。若同时提示
  “回滚一致点已确认旧译文完整恢复”，表示正式译文已在该一致点恢复，manifest 与
  源分块未更新；打印的安全归档仍须保留供核对，不得自动删除。若没有归档路径，
  则失败发生在建立首个私有载体之前。外部进程仍可能在一致点之后修改公开路径。
- 旧版 `manifest.json` 与 `.zh.txt`：禁止直接合并，因为没有 hash 就无法证明译文
  ...
  人工移走这些路径条目再重试。
- `无法隔离旧版译文`：隔离失败，但没有检测到部分回滚状态。若同时打印安全归档，
  表示工具已在回滚一致点确认旧 `.zh.txt` 完整恢复；归档仍会原样保留供核对，不能
  因回滚完整而自动删除。若没有归档路径，则失败发生在建立首个私有载体之前。
  manifest 与源分块均未更新；修复问题后重试。
- `旧版译文只完成了部分隔离`：隔离失败，且工具检测到 source/stale 身份变化或
  无法完整恢复可见旧译文路径；manifest 与源分块仍未更新。已建立的私有载体会
  原样保留，但不保证每个失败归档都是旧译文。不要继续翻译或合并；逐项核对
  `.zh.txt`、`stale-legacy` 与提示的 `.srt-safety-archive-*`。竞争文件不得删除或
  覆盖；确认内容归属、修复权限后再运行 chunk。
