...
2. 流水线中复用已解析的 prepare JSON，将 `item_dir` 和 `native_lang`
...
- exit 0：复用返回的同一 `ITEM_DIR`，使用 `NATIVE_LANG` 生成正文。
...
```bash
python -c 'from pathlib import Path; import sys; p=Path(sys.argv[1]); ok=p.is_file() and p.stat().st_size>0; print("文件存在且非空" if ok else "文件缺失或为空"); raise SystemExit(not ok)' "$ITEM_DIR/digest.md"
```
...
```bash
python -c 'from pathlib import Path; import sys; lines=Path(sys.argv[1]).read_text(encoding="utf-8").splitlines(); hs=["## 一句话核心","## 核心要点","## 原文金句"]; ok=all(lines.count(h)==1 for h in hs); print("摘要结构通过" if ok else "摘要结构缺失或重复"); raise SystemExit(not ok)' "$ITEM_DIR/digest.md"
```
