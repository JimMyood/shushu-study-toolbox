...
2. 根据本次任务判断自检结果。exit 0 表示全部检查通过；exit 1 表示至少一项未通过，不代表所有能力都不可用。仅 `faster-whisper` 失败时，只停止本地转写，下载、来源字幕、翻译、嵌字幕、笔记和资料精读仍可按各自依赖继续。恢复转写时必须采用 doctor 针对当前解释器给出的指引：Python 3.10–3.13 在已激活 venv 中安装依赖，范围外则选择该范围内的解释器重建。完整阅读目标工具手册，逐项处理本次任务真正需要的依赖。
